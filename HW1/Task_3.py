...
def Self_driving_path_planner(world_map, start_pos, goal_pos):
    """
    Plan a path using Hybrid A* with Dubins heuristic and analytic expansion.

    Arguments:
    world_map -- 120x120 array, 0 free, 1 obstacle
    start_pos -- [x, y]
    goal_pos -- [x, y]
    ...
    if raw_path:
        ...
        t1 = time.time()
        smooth = dubins_shortcut_smooth(world_map, raw_path, radius=radius, step=step,
                                        iterations=120, min_gap=8, seed=42)
        dt2 = (time.time() - t1) * 1000.0
        post_len = path_length(smooth)
...
if __name__ == '__main__':
    ...
    # Visualize the map and path with heading arrows.
    ...
    path = np.array(path) if path else np.empty((0,3))
    plt.figure(figsize=(8,8))
    if path.size:
        plt.plot(path[:,0], path[:,1], '-r', lw=1.0, label='Hybrid A* path')
        headings = path[:,2].astype(float)
        dx = np.cos(np.radians(90.0 - headings))
        dy = np.sin(np.radians(90.0 - headings))
        # 稍微稀疏抽样箭头，避免太密
        step_q = max(1, len(path)//100)
        plt.quiver(path[::step_q,0], path[::step_q,1], dx[::step_q], dy[::step_q],
                   color='r', scale=15, width=0.005)

    plt.plot(start_pos[0], start_pos[1], 'og', label='start')
    plt.plot(goal_pos[0], goal_pos[1], 'xb', label='goal')
    plt.plot(obstacles_x, obstacles_y, '.k', ms=2)
    plt.grid(True)
    plt.axis('equal')
    plt.legend()
    plt.title('Hybrid A* with Dubins analytic expansion')
    plt.show()
